...
useEffect(() => {
    const fetchCustomers2 = () => {
        let posts = [];
        //const refProducts = firebase.firestore().collection('users').orderBy('name','desc').get()
        const refProducts = firebase.firestore().collection('products').orderBy('name','asc').get()
        .then(snapshot => {
          if (snapshot.empty) {
