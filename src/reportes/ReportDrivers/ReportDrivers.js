...
useEffect(() => {
    const fetchCustomers2 = () => {
        ...
        const refUsers = firebase.firestore().collection('drivers').orderBy('name','asc').get()
        .then(snapshot => {
          if (snapshot.empty) {
      ...
      fetchCustomers2();
        ...
        return (
            <ExcelFile element={<button style={{backgroundColor:colors.blue['600'],color:"#ffffff", marginRight:"10px"}} class="MuiButtonBase-root MuiButton-root makeStyles-deleteButton-456 MuiButton-text">Exportar {t("drivers")}</button>}>
