...
const PromotionEdit = props => {
  const { open, onClose, promotion, cboRestaurants, chips, actualizar, className, ...rest } = props;
  ...
  const [formState, setFormState] = useState({
    ...promotion, restaurantID, places, restaurantName,cboRestaurants
  });
  ...
  const handleSave = () => {
    ...
    let msg = "Promoción actualizada exitosamente!";
    if(isValid){
    ...
    fetch(service+'promotionUpdateAdmin', {
        method: 'post',
        mode: 'cors',
          ...
          console.log(body);
           ...
           uploadDoc.on('state_changed', function (snapshot) {
             ...
             uploadDoc.snapshot.ref.getDownloadURL().then(function (downloadURL) {
               ...
               console.log(data);
              const refWasher = firebase.firestore().collection('promos').doc(promotion.id);
...
PromotionEdit.displayName = 'PromotionEdit';
