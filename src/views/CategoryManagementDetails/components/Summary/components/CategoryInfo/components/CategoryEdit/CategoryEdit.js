...
const CategoryEdit = props => {
  const { open, onClose, category, actualizar, className, ...rest } = props;
  ...
  const handleSave = () => {
    ...
    let msg = "Category actualizado exitosamente!";
    if(isValid){
    ...
    fetch(service+'categoryUpdateAdmin', {
        method: 'post',
        mode: 'cors',
          ...
          console.log(body);
           ...
           uploadDoc.on('state_changed', function (snapshot) {
             ...
             uploadDoc.snapshot.ref.getDownloadURL().then(function (downloadURL) {
               ...
               console.log(data);
              const refWasher = firebase.firestore().collection('categories').doc(category.id);
