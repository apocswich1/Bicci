...
const AdministratorManagementList = () => {
  ...
  const [administratorsBkp, setAdministratorsBkp] = useState([]);
...
console.log(data);
  ...
  useEffect(() => {
    ...
    const fetchAdministrators = () => {
      fetch(service+'listAdministratorsAdmin', {
        method: 'get',
        mode: 'cors',
      }).then(function (respuesta) {
        respuesta.json().then(async body => {
        
          for(let i=0;i<body.usuarios.length; i++){
            let ref = await firebase.firestore().collection('restaurants').where('starredBy','==',body.usuarios[i].id).get();
            let res = await ref.docs.map(item => {return item.data()});
            console.log(res);
          }
          setAdministrators(body.usuarios.filter(item => item.deleted !== true));
          setAdministratorsBkp(body.usuarios.filter(item => item.deleted !== true));
  ...
  const handleSearch = () => { 
    ...
    if(search === "" || search == undefined){
      actualizar();
      finded = [];
      setAdministrators(administratorsBkp);
      setAdministratorsBkp(administratorsBkp);
    
    }else{ 
    
    let words = administratorsBkp.filter((item) => {
...
export default AdministratorManagementList;
