...
const AvatarEdit = props => {
  const { open, onClose, restaurant, actualizar, className, ...rest } = props;
  ...
  const handleSave = () => {
    ...
    let body = {id: restaurant.id, code: 200}
    if(isValid){

     /************* imagen de producto*/
     if(filess.length > 0){
      var storageRef = firebase.storage().ref();
     var uploadDoc = storageRef.child(`/thumbnails_restaurant/${restaurant.id}/${restaurant.id}.jpg`).put(filess[0]);
     uploadDoc.on('state_changed', function (snapshot) {
       ...
       uploadDoc.snapshot.ref.getDownloadURL().then(function (downloadURL) {
         ...
         console.log(data);
        const refWasher = firebase.firestore().collection('restaurants').doc(restaurant.id);
