...
const KitchenProgramados = () => {
  ...
  useEffect(() => {
    ...
    let doc = firebase.firestore().collection('orders').where('status', '==', 1).where('scheduled','==',true);
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        setOrdenes(respuesta.filter(item => { return item.enCocina !== true && item.status == 2 && item.scheduled == true}));
      ...
      >
        ...
        <Grid item lg={12} xs={12}>
          ...
          />
             ...
             <Detalle orden={orden} scheduled={scheduled} setOpenDetalle={setOpenDetalle} ordenes={ordenes} setOrden={Actualizar} grocery={grocery} valuePestana={valuePestana} />
          ))}
        </Grid>
        ):("")}
        {openDetalle===true ? (
...
export default KitchenProgramados;
