...
const DetalleOrden = props => {
  ...
  const enviarCocina = async () => {
    ...
    .doc(orden.id).set({enCocina: true},{merge: true}); 
    fetchData();
      ...
      >
        ...
        <div className={classes.statsContainer} key={orden.id}>
          ...
          <div className={classes.statsItem2}>
            ...
            <Typography align="right" variant="h1" style={{ paddingTop: "10px",marginRight:"5px", fontSize:"28px" }}>{tiempo.hora(orden.date,15)}</Typography>
          </div>
        </div>
