...
import { DetalleOrden, Ordenes, DetalleCliente, OrderActivity,OrderIngredients, DetalleRepartidor } from '../../components'
...
const Detalle = props => {
  ...
  const { orden, valuePestana, ordenes, ingredients, setOpenDetalle, setOrden, grocery } = props;
    ...
    <div className={classes.root}>
      <AppBar position="static" color="default">
        ...
        >
          ...
          <Tab label="Ingredientes Extras" {...a11yProps(1)} />
        </Tabs>
      </AppBar>
      <SwipeableViews
              ...
              <Table stickyHeader aria-label="sticky table">
                <TableBody>
                  {ordenes.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    return (
                      <TableRow style={{ height: "70px" }} role="checkbox" tabIndex={-1} key={row.id}>
                        {columns.map((column) => {
        ...
        <TabPanel value={value} index={1} dir={theme.direction}>
          <Paper className={classes.root}>
            <TableContainer className={classes.container}>
              <Table stickyHeader aria-label="sticky table">
                <TableBody>
                  {ingredients.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    return (
                      <TableRow style={{ height: "70px" }} role="checkbox" tabIndex={-1} key={row.id}>
                        {columns.map((column) => {
                          const value = row[column.id];
