...
const KitchenHistorial = () => {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        setOrdenes(respuesta.filter(item => { return item.status == 6}));
  ...
  const fetchDataOrden = async () => {
    try {
      ...
      const ordenesRef = await firebase.firestore().collection("orders").orderBy('date', 'desc').get();
      let respuesta = await ordenesRef.docs.map((item) => { return item.data() });
      setOrdenes(respuesta.filter(item => { return item.status >= 6 && item.status <= 9}));
...
export default KitchenHistorial;
