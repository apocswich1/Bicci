...
const DocumentManagementList = () => {
  ...
  const [documentsBkp, setDocumentsBkp] = useState([]);
...
console.log(data);
    ...
    const fetchDocuments = () => {
      fetch(service+'listDocumentsAdmin', {
        ...
        respuesta.json().then(body => {
          setDocuments(body.usuarios.filter(item => item.deleted !== true));
          setDocumentsBkp(body.usuarios.filter(item => item.deleted !== true));
  ...
  const handleSearch = () => { 
    ...
    if(search === "" || search == undefined){
      actualizar();
      finded = [];
      setDocuments(documentsBkp);
      setDocumentsBkp(documentsBkp);
    
    }else{ 
    
    let words = documentsBkp.filter((item) => {
  ...
  return (
    <Page
      ...
      title="Document Management List"
    >
      <Snackbar
