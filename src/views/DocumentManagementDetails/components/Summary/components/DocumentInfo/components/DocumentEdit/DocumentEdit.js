...
const DocumentEdit = props => {
  const { open, onClose, document, actualizar, className, ...rest } = props;
  ...
  const validateForm = () => {
    ...
    if(!formState.kind){
      kindError = "Debe introducir el tipo";
      kindErrorMessage = "Debe introducir el tipo";
    }
    ...
    if(nameError){
      setFormState(formState => ({
        ...formState,
        nameError,nameErrorMessage,kindError,kindErrorMessage
      }));
      return false;
    }
    return true;
  ...
  const handleSave = () => {
    ...
    let msg = "Document actualizado exitosamente!";
    if(isValid){
    ...
    fetch(service+'documentUpdateAdmin', {
        method: 'post',
        mode: 'cors',
        ...
        respuesta.json().then(body => {
          console.log(body);
           ...
           var uploadDoc = storageRef.child(`/thumbnails/${id}/${id}.jpg`).put(filess[0]);
           uploadDoc.on('state_changed', function (snapshot) {
             ...
             uploadDoc.snapshot.ref.getDownloadURL().then(function (downloadURL) {
               ...
               console.log(data);
              const refWasher = firebase.firestore().collection('documents').doc(id);
          ...
          /************* */
            ...
            if(filessdoc.length > 0){
             ...
             var uploadDocument = storageReference.child(`/principalDocs/${id}/${id}.pdf`).put(filessdoc[0]);
             uploadDocument.on('state_changed', function (snapshot) {
               ...
               uploadDoc.snapshot.ref.getDownloadURL().then(function (downloadURL) {
                 ...
                 console.log(datadoc);
                const refPro = firebase.firestore().collection('documents').doc(id);
