...
const KitchenAdd = props => {
  const { open, onClose, kitchen, actualizar, setLoading, className, cboCategories, ...rest } = props;
  ...
  const validateForm = () => {
    ...
    // if(!formState.kitchenLine){
    //   kitchenLineError = "Debe introducir la kitchenLine";
    //   kitchenLineErrorMessage = "Debe introducir la kitchenLine";
    // }
    ...
    if(nameError || actionError || categoryError || brandError || compositionError || concentrationError || descriptionError || deseasesError || skuError || kitchenLineError || quantityError || substanceError || presentationError){
      
      setFormState(formState => ({
        ...formState,
        nameError,actionError,categoryError,brandError,compositionError,concentrationError,descriptionError,deseasesError,skuError,kitchenLineError,quantityError,substanceError,
        ...
        posologyError,posologyErrorMessage,treatmentError,treatmentErrorMessage,presentationError,presentationErrorMessage,kitchenLineErrorMessage,quantityErrorMessage,substanceErrorMessage
      }));
      console.log(formState);
      return false;
    }
  ...
  const handleSave = (event) => {
    ...
    console.log(formState);
    let msg = "Kitchens creado con exito";
    ...
    if(isValid){
      ...
      let params = {
        ...
        kitchenLine:formState.kitchenLine,
      ...
      fetch(service+'createKitchensAdmin', {
          method: 'post',
          mode: 'cors',
          ...
          respuesta.json().then(body => {
            console.log(body);
               ...
               var uploadDoc = storageRef.child(`/thumbnails_kitchen/${body.id}/${body.id}.jpg`).put(filess[0]);
               uploadDoc.on('state_changed', function (snapshot) {
                 ...
                 uploadDoc.snapshot.ref.getDownloadURL().then(function (downloadURL) {
                   ...
                   console.log(data);
                  const refWasher = firebase.firestore().collection('kitchens').doc(body.id);
            ...
            if(filessdoc.length > 0){
             ...
             uploadDocument.on('state_changed', function (snapshot) {
               ...
               uploadDoc.snapshot.ref.getDownloadURL().then(function (downloadURL) {
                 ...
                 console.log(datadoc);
                const refPro = firebase.firestore().collection('kitchens').doc(body.id);
            ...
            /************* */
             ...
             if(filesspromo[0]!==undefined){
              ...
              var storageRef = firebase.storage().ref();
              var uploadDoc = storageRef.child(`/thumbnails_kitchen_promo/${body.id}/${body.id}.jpg`).put(filesspromo[0]);
              uploadDoc.on('state_changed', function (snapshot) {
                var progress = (snapshot.bytesTransferred / snapshot.totalBytes) * 100;
                console.log('Upload is ' + progress + '% done');
                switch (snapshot.state) {
          ...
          <CardContent>
            ...
            >
              ...
              >
                <TextField
                  fullWidth
                  label={T("Kitchen Line")}
                  name="kitchenLine"
                  onChange={handleFieldChange}
                  value={formState.kitchenLine}
                  variant="outlined"
                  error={formState.kitchenLineError}
                  helperText={formState.kitchenLineErrorMessage}
                  // InputProps={{
...
KitchenAdd.displayName = 'KitchenAdd';

KitchenAdd.propTypes = {
