...
const KitchenEditPrice = props => {
  const { open, onClose, kitchen, actualizar,cruzverde, kitchenPrice, salcobrand,ahumada, className, cboCategories, ...rest } = props;
  ...
  const handleSave = async () => {
    ...
    // const sal = await firebase.firestore().collection('kitchens').doc(kitchenid).collection("storeKitchens").where("storeID","==","bUi3aSG0IM9qpoIx8rHM");
    // sal.set({price: params[0].price, discount:params[0].discount}, {merge: true});
    // const ahu = await firebase.firestore().collection('kitchens').doc(kitchenid).collection("storeKitchens").where("storeID","==","CW3GQ4Hj8Se7Xfnx0Jne");
    // ahu.set({price: params[0].price, discount:params[0].discount}, {merge: true});
    // const cruz = await firebase.firestore().collection('kitchens').doc(kitchenid).collection("storeKitchens").where("storeID","==","x7JmmpuBRD5s7QLMDwIW");
    // cruz.set({price: params[0].price, discount:params[0].discount}, {merge: true});
    
    const ref = await firebase.firestore().collection('kitchens').doc(kitchenid).collection("storeKitchens").get();
    const ref2 = firebase.firestore().collection('kitchens').doc(kitchenid).collection("storeKitchens");
