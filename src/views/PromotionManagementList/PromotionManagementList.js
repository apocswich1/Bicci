...
const PromotionManagementList = () => {
  const classes = useStyles();
  const [cboRestaurants, setCboRestaurants] = React.useState([]);
  const [promotions, setPromotions] = useState([]);
  const [promotionsBkp, setPromotionsBkp] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchPromotions = async () => {
      try {
        let refPromo = await firebase.firestore().collection("promos").get();
        let result = await refPromo.docs.map(item => { return item.data() });
        setPromotions(result.filter(item => item.deleted !== true));
        setPromotionsBkp(result.filter(item => item.deleted !== true));
      ...
      // fetch(service+'listPromotionsAdmin', {
      //   method: 'get',
      //   mode: 'cors',
      // }).then(function (respuesta) {
      //   respuesta.json().then(body => {
      //     setPromotions(body.usuarios.filter(item => item.deleted !== true));
      //     setPromotionsBkp(body.usuarios.filter(item => item.deleted !== true));
  ...
  const handleSearch = () => {
    ...
    if (search === "" || search == undefined) {
      actualizar();
      finded = [];
      setPromotions(promotionsBkp);
      setPromotionsBkp(promotionsBkp);

    } else {

      let words = promotionsBkp.filter((item) => {
  ...
  return (
    <Page
      ...
      title="Promotion Management List"
    >
      <Snackbar
      ...
      <Header actualizar={actualizar} setLoading={setLoading} cboRestaurants={cboRestaurants} />
      <SearchBar
