...
const StoreManagementList = () => {
  ...
  const [storesBkp, setStoresBkp] = useState([]);
...
console.log(data);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    const fetchStores = () => {
      fetch(service+'listStoresAdmin', {
        method: 'get',
        mode: 'cors',
      }).then(function (respuesta) {
        respuesta.json().then(body => {
          setStores(body.usuarios.filter(item => item.deleted !== true));
          setStoresBkp(body.usuarios.filter(item => item.deleted !== true));
  ...
  const handleSearch = () => { 
    ...
    if(search === "" || search == undefined){
      ...
      finded = [];
      setStores(storesBkp);
      setStoresBkp(storesBkp);
    
    }else{ 
    
    let words = storesBkp.filter((item) => {
