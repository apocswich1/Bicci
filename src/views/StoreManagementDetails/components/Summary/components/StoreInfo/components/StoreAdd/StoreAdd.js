...
const StoreAdd = props => {
  const { open, onClose, store, actualizar, setLoading, className, ...rest } = props;
  ...
  const handleSave = (event) => {
    ...
    let msg = "Stores creado con exito";
    ...
    if(isValid){
      ...
      fetch(service+'createStoresAdmin', {
          method: 'post',
          mode: 'cors',
            ...
            console.log(body);
             ...
             uploadDoc.on('state_changed', function (snapshot) {
               ...
               uploadDoc.snapshot.ref.getDownloadURL().then(function (downloadURL) {
                 ...
                 console.log(data);
                const refWasher = firebase.firestore().collection('stores').doc(body.id);
