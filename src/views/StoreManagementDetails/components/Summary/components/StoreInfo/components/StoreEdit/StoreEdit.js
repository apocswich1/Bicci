...
const StoreEdit = props => {
  const { open, onClose, store, actualizar, className, ...rest } = props;
  ...
  const handleSave = () => {
    ...
    let msg = "Store actualizado exitosamente!";
    if(isValid){
    ...
    fetch(service+'storeUpdateAdmin', {
        method: 'post',
        mode: 'cors',
          ...
          console.log(body);
           ...
           uploadDoc.on('state_changed', function (snapshot) {
             ...
             uploadDoc.snapshot.ref.getDownloadURL().then(function (downloadURL) {
               ...
               console.log(data);
              const refWasher = firebase.firestore().collection('categories').doc(store.id);
