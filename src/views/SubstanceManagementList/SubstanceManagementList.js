...
const SubstanceManagementList = () => {
  const classes = useStyles();

  const [substances, setSubstances] = useState([]);
  const [substancesBkp, setSubstancesBkp] = useState([]);
...
console.log(data);
  ...
  useEffect(() => {
    ...
    const fetchSubstances = () => {
      fetch('https://us-central1-deutschpharma-5edbb.cloudfunctions.net/listSubstancesAdmin', {
        method: 'get',
        mode: 'cors',
      }).then(function (respuesta) {
        respuesta.json().then(body => {
          setSubstances(body.usuarios.filter(item => item.deleted !== true));
          setSubstancesBkp(body.usuarios.filter(item => item.deleted !== true));
  ...
  const handleSearch = () => { 
    ...
    if(search === "" || search == undefined){
      actualizar();
      finded = [];
      setSubstances(substancesBkp);
      setSubstancesBkp(substancesBkp);
    
    }else{ 
    
    let words = substancesBkp.filter((item) => {
  ...
  return (
    <Page
      ...
      title="Substance Management List"
    >
      <Snackbar
...
export default SubstanceManagementList;
