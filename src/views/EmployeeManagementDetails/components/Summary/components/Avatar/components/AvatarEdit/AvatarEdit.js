...
const AvatarEdit = props => {
  ...
  const handleSave = () => {
    ...
    let body = {id: employee.uid, code: 200}
    if(isValid){

     /************* imagen de producto*/
     if(filess.length > 0){
      var storageRef = firebase.storage().ref();
     var uploadDoc = storageRef.child(`/thumbnails/${employee.uid}/${employee.uid}.jpg`).put(filess[0]);
     uploadDoc.on('state_changed', function (snapshot) {
       ...
       uploadDoc.snapshot.ref.getDownloadURL().then(function (downloadURL) {
         ...
         console.log(data);
        const refWasher = firebase.firestore().collection('employees').doc(employee.uid);
