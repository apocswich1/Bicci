...
const AvatarEdit = props => {
  const { open, onClose, claim, actualizar, className, ...rest } = props;
  ...
  const handleSave = () => {
    ...
    let body = {id: claim.id, code: 200}
    if(isValid){

     /************* imagen de producto*/
     if(filess.length > 0){
      var storageRef = firebase.storage().ref();
     var uploadDoc = storageRef.child("/thumbnails/"+claim.id+"/"+claim.id+".jpg").put(filess[0]);
     uploadDoc.on('state_changed', function (snapshot) {
       ...
       uploadDoc.snapshot.ref.getDownloadURL().then(function (downloadURL) {
         ...
         console.log(data);
        const refWasher = firebase.firestore().collection('claims').doc(claim.id);
