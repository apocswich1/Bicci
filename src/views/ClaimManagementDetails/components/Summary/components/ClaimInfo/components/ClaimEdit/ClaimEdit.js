...
const ClaimEdit = props => {
  const { open, onClose, $variablemodulo, actualizar, className, ...rest } = props;
  ...
  const handleSave = () => {
    ...
    let msg = "Claim actualizado exitosamente!";
    if(isValid){
    ...
    fetch(service+'$variablemoduloUpdateAdmin', {
        method: 'post',
        mode: 'cors',
          ...
          console.log(body);
           ...
           uploadDoc.on('state_changed', function (snapshot) {
             ...
             uploadDoc.snapshot.ref.getDownloadURL().then(function (downloadURL) {
               ...
               console.log(data);
              const refWasher = firebase.firestore().collection('$variablemodulos').doc($variablemodulo.id);
    ...
    >
      ...
      >
        <form>
          <CardContent>
            <Typography
              align="$variablemodulo"
              gutterBottom
              variant="h3"
            >
              {t("Edit")} {t("$variablemodulo")}
            </Typography>
            <Grid
...
ClaimEdit.displayName = 'ClaimEdit';

ClaimEdit.propTypes = {
  className: PropTypes.string,
  $variablemodulo: PropTypes.any,
  onClose: PropTypes.func,
  open: PropTypes.bool
};

ClaimEdit.defaultProps = {
  open: false,
  onClose: () => {}
};
