...
const KindEdit = props => {
  const { open, onClose, kind, restaurantID, actualizar, className, ...rest } = props;
  ...
  const handleSave = () => {
    ...
    let msg = "Kind actualizado exitosamente!";
    if(isValid){
    ...
    fetch(service+'categoryProductUpdateAdmin', {
        method: 'post',
        mode: 'cors',
          ...
          console.log(body);
           ...
           uploadDoc.on('state_changed', function (snapshot) {
             ...
             uploadDoc.snapshot.ref.getDownloadURL().then(function (downloadURL) {
               ...
               console.log(data);
              const refWasher = firebase.firestore().collection('kinds').doc(kind.id);
