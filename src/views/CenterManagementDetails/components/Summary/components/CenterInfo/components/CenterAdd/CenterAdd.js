...
const CenterAdd = props => {
  const { open, onClose, center, company, actualizar, setLoading, className, ...rest } = props;
  ...
  const handleSave = (event) => {
    ...
    let msg = "Centers creado con exito";
    ...
    if(isValid){
      ...
      fetch(service+'createCentersAdmin', {
          method: 'post',
          mode: 'cors',
            ...
            console.log(body);
             ...
             uploadDoc.on('state_changed', function (snapshot) {
               ...
               uploadDoc.snapshot.ref.getDownloadURL().then(function (downloadURL) {
                 ...
                 console.log(data);
                const refWasher = firebase.firestore().collection('centers').doc(body.id);
...
CenterAdd.displayName = 'CenterAdd';
