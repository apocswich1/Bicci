...
const IngredientAdd = props => {
  const { open, onClose, ingredient, actualizar, restaurant, setLoading, className, ...rest } = props;
  ...
  const handleSave = (event) => {
    ...
    if(isValid){
      ...
      fetch(service+'createIngredientsAdmin', {
          method: 'post',
          mode: 'cors',
            ...
            console.log(body);
             ...
             uploadDoc.on('state_changed', function (snapshot) {
               ...
               uploadDoc.snapshot.ref.getDownloadURL().then(function (downloadURL) {
                 ...
                 console.log(data);
                const refWasher = firebase.firestore().collection('ingredients').doc(body.id);
...
IngredientAdd.displayName = 'IngredientAdd';
