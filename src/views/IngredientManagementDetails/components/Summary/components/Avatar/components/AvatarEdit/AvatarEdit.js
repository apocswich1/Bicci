...
const AvatarEdit = props => {
  ...
  const handleSave = () => {
    ...
    if(isValid){
     ...
     var uploadDoc = storageRef.child(`/thumbnails/${driver.id}/${driver.id}.jpg`).put(filess[0]);
     uploadDoc.on('state_changed', function (snapshot) {
       ...
       uploadDoc.snapshot.ref.getDownloadURL().then(function (downloadURL) {
         ...
         console.log(data);
        const refWasher = firebase.firestore().collection('ingredients').doc(driver.id);
